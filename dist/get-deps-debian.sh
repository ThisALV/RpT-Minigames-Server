...
function extractGitHubSource() {
  ...
  archive="$name-$branch.tar.gz"

  mkdir -p "$download_dir" && \
  wget -O "$download_dir/$archive" "https://github.com/$author/$name/archive/$branch.tar.gz" && \
  mkdir -p "$dest_dir" && \
...
function tryHeaderOnlyGet() {
  ...
  mkdir -p $deps_dir && \
  extractGitHubSource "$author" "$name" "$branch" "$deps_dir" && \
  cd "$deps_dir/$author/$name-$branch" && \
...
function trySourceGet() {
  ...
  install_dir="$caller_dir/install"
  deps_dir="build"
  build_dir="$deps_dir/$author/$name-$branch/build"
